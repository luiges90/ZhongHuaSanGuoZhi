namespace GameObjects
{
    ...
    public class Architecture : GameObject
    {
        ...
        private PersonList SelectSubOfficersToTroop(Troop t)
        {
            ...
            {
                if ((!person.Selected && !t.Persons.HasGameObject(person)) && t.Leader.FightingForce < person.SubFightingForce
                    && t.Leader.FightingForce > t.Leader.SubFightingForce && person.SubFightingForce > person.FightingForce
                    && t.Leader.FightingForce > person.FightingForce)
                {
                    person.Selected = true;
                    result.Add(person);
                    personCnt++;
                    if (personCnt >= 3) break;
                }
                else if (t.Leader.FightingForce <= t.Leader.SubFightingForce) break;
            }
            if (t.TroopIntelligence < 80)
            {
                foreach (Person person in candidates)
                {
                    ...
                    if (!person.Selected && person.Intelligence >= 80 && person.Strength < t.TroopStrength && person.Intelligence - t.TroopIntelligence >= 10 && person.FightingForce < t.Leader.FightingForce)
                    {
                        person.Selected = true;
                        result.Add(person);
                        personCnt++;
            ...
            if (t.TroopStrength < 75)
            {
                ...
                {
                    if (person.WaitForFeiZi != null) continue;
                    if (!person.Selected && person.Strength >= 75 && person.ClosePersons.IndexOf(t.Leader.ID) >= 0 && person.Strength - t.TroopStrength >= 10 && person.FightingForce < t.Leader.FightingForce)
                    {
                        person.Selected = true;
                        result.Add(person);
                        personCnt++;
            ...
            if (t.TroopCommand < 75)
            {
                ...
                {
                    if (person.WaitForFeiZi != null) continue;
                    if (!person.Selected && person.Command >= 75 && person.ClosePersons.IndexOf(t.Leader.ID) >= 0 && person.Command - t.TroopCommand >= 10 && person.FightingForce < t.Leader.FightingForce)
                    {
                        person.Selected = true;
                        result.Add(person);
                        personCnt++;
        ...
        */
        ...
        private Troop BuildOffensiveTroop(Architecture destination, LinkKind linkkind, bool offensive)
        {
            ...
            if (list.Count > 0)
            {
                ...
                {
                    if (troop2.SimulatingFightingForce < 10000 && offensive)
                    {
                        break;
                    }
        ...
        private Troop AISelectPersonIntoTroop_inner(Person leader, Military military, Architecture from)
        {
            PersonList singleton = new PersonList();
            singleton.Add(leader);
            Troop t = Troop.CreateSimulateTroop(singleton, military, from.Position);

            if (t.Leader.preferredTroopPersons.Count > 0)
            {
                foreach (Person p in t.Leader.preferredTroopPersons)
                {
                    bool allPersonHere = true;
                    if (p.LocationArchitecture != from) 
                    {
                        allPersonHere = false;
                    }
                    if (allPersonHere) 
            ...
            PersonList pl = this.SelectSubOfficersToTroop(t);
            t.Candidates = pl;
            foreach (Person i in t.Candidates)
            {
                if (i != t.Leader)
                {
                    t.Leader.preferredTroopPersons.Add(i);
                }
            }
        ...
        private TroopList AISelectPersonIntoTroop(Architecture from, Military military)
        {
            ...
            {
                result.Add(AISelectPersonIntoTroop_inner(military.FollowedLeader, military, from));
            }
            ...
            {
                result.Add(AISelectPersonIntoTroop_inner(military.Leader, military, from));
            }
            else
            {
                foreach (Person person in from.Persons)
                {
                    if (!person.Selected)
                    {
                        ...
                        {
                           result.Add(AISelectPersonIntoTroop_inner(person, military, from));
                        }
                    }
                }
            }
        ...
        private void DefensiveCampaign()
        {
            ...
            if ((this.HasPerson() && this.HasCampaignableMilitary()) && (this.GetAllAvailableArea(false).Count != 0))
            {
                if (hostileTroopsInView.Count > 0)
                {
                    ...
                    {
                        list4.IsNumber = true;
                        list4.PropertyName = "SimulatingFightingForce";
                        list4.ReSort();
                ...
                foreach (LinkNode i in this.AIAllLinkNodes.Values)
                {
                        ...
                        && i.A.BelongedSection.AIDetail.AutoRun)
                    {
                        ...
                        if (supportList.Count > 0)
                        {
                            ...
                            supportList.PropertyName = "SimulatingFightingForce";
                            supportList.ReSort();
