    ...
    public class Architecture : GameObject
    {
        ...
        */
        private Troop BuildTransportTroop(Architecture destination, Military military, int food, int fund)
        {
            ...
            troop = this.CreateTroop(leader, leader[0] as Person, military, food, this.Position);
                ...
                if (!this.HasHostileTroopsInView())
                {
                    ...
                    if ((this.HasPerson() && this.HasCampaignableMilitary()) && (this.GetAllAvailableArea(false).Count != 0))
                    {
                        if ((((GameObject.Random(GameObject.Square(((int)this.BelongedFaction.Leader.StrategyTendency) + 1)) == 0) && ((this.IsFoodAbundant || (this.IsFoodEnough && GameObject.Chance(0x21))) || GameObject.Chance(5))) && (((base.Scenario.Date.Season != GameSeason.冬) || GameObject.Chance(5)) && (!this.HostileLine || GameObject.Chance(10)))) && ((GameObject.Chance(20) && this.BelongedSection != null && this.BelongedSection.AIDetail.AllowMilitaryTransfer) || (this.BelongedSection != null && this.BelongedSection.AIDetail.ValueOffensiveCampaign && GameObject.Chance(50))))
                        {
                            ...
                            //then, move military to most troops whereas there has troops less than large army scale, ...
            ...
            if (this.Population > 0)
            {
                ...
                using (enumerator = this.BelongedFaction.AvailableMilitaryKinds.MilitaryKinds.Values.GetEnumerator())
                {
                    while (enumerator.MoveNext())
                    {
                        ...
                        if ((((this.ValueWater == (current.Type == MilitaryType.水军)) || (!water && GameObject.Chance(20))) && current.CreateAvail(this)) && (current.ID != 0x1d))
                        {
                            list.Add(current);
                        }
                    }
        ...
        */
        ...
        private void AIPersonTransfer()
        {
            ...
            if (!this.HasAnyPerson() && !this.CompletelyDeveloped && !(this.RecentlyAttacked > 0 && this.Endurance <= 0))
            {
                ...
                {
                    if (p.LocationArchitecture != null && p.Merit < minMerit && p.BelongedArchitecture.PersonCount + p.BelongedArchitecture.MovingPersons.Count > 1)
                    {
                        personToMove = p;
                        minMerit = p.Merit;
                    }
                ...
                {
                    personToMove.BelongedArchitecture.RemovePerson(personToMove);
        ...
        */
        ...
        private Troop BuildOffensiveTroop(Architecture destination, LinkKind linkkind, bool offensive)
        {
            ...
            foreach (Military military in this.Militaries.GetRandomList())
            {
                ...
                if ((((military.Scales >= 3) && (military.Morale >= 80)) && (military.Combativity >= 80)) && (military.InjuryQuantity < military.Kind.MinScale))
                {
                    ...
                    if ((((military2.Leader != null) && (military2.LeaderExperience >= 10)) && (((military2.Leader.Strength >= 80) || (military2.Leader.Command >= 80)) || military2.Leader.HasLeaderValidCombatTitle)) && this.Persons.HasGameObject(military2.Leader))
                    {
                        list2 = new PersonList();
            ...
            if (list.Count > 0)
            {
                ...
                {
                    if (troop2.FightingForce < 0x2710 && offensive)
                    {
                        break;
                    }
        ...
        private void captiveEscape()
        {
            ...
            {
                if (GameObject.Random((this.Domination * 10 + this.Morale) * 30)+200 <= GameObject.Random(p.CaptivePerson.CaptiveAbility))
                {
                    p.CaptiveEscape();
                    break;
                }
        ...
        private void zainanshijian()
        {
            ...
            {
                if (GameObject.Random(GlobalVariables.zainanfashengjilv)==0 && base.Scenario.IsPlayer(this.BelongedFaction) && this.Kind.ID == 1)
                {
                    int kindID;
        ...
        private void AIResourceTransfer()
        {
            ...
            //transfer when
            //1. there are person here
            //2. food or fund is adundant here and here is not frontline nor hostile line
            //3. or, food and fund is reaching the ceiling
            //4. or, the city is falling (endurance = 0 or endurance < 30 with 33% chance, and recently attacked)
            if ((((this.IsFoodAbundant || this.IsFundAbundant) && !this.HostileLine && !this.FrontLine)
                || (this.Food > this.FoodCeiling / 10 * 9 || this.Fund > this.FundCeiling / 10 * 9)
                || ((this.Endurance < 30) && this.RecentlyAttacked > 0)))
            {
                if (!this.HasPerson())
                {
                ...
                else if (GameObject.Chance(20))
                {
                    ...
                    int transferFood = Math.Max(Math.Min(!this.HostileLine && !this.FrontLine ? this.Food - this.EnoughFood : this.Food - this.FoodCeiling / 4 * 3, dest.A.FoodCeiling / 10 * 9 - dest.A.Food), 0);
                    int transferFund = Math.Max(Math.Min(!this.HostileLine && !this.FrontLine ? this.Fund - this.EnoughFund : this.Fund - this.FundCeiling / 4 * 3, dest.A.FundCeiling / 10 * 9 - dest.A.Fund), 0);
