namespace GameObjects.Influences.InfluenceKindPack
{
    ...
    internal class InfluenceKind391 : InfluenceKind
    {
        private int maxDays;
        ...
        public override int GetCredit(Troop source, Troop destination)
        {
            ...
            foreach (Troop troop in source.GetAreaStratagemTroops(destination, false))
            {
                ...
                if (num3 > 0)
                {
                    num3 = ((num3 + ((troop.Army.Scales - 5) * 5)) * troop.FightingForce) / pureFightingForce;
                    if (troop.Status == TroopStatus.混乱)
                    {
                        num3 /= GameObject.Square(source.ViewingHostileTroopCount) + 2;
                    }
                    num += num3;
                }
            }
