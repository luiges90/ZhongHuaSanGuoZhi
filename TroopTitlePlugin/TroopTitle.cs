...
/*
        ...
        {get
            {
                if (File.Exists(@"GameComponents\TroopTitle\Data\PersonPicture\" + this.PersonID.ToString() + ".png"))
                { return true; }
                else
                { return false; }
            }
        ...
        {get
            {
                if (File.Exists(@"GameComponents\TroopTitle\Data\FactionPicture\" + this.FactionID.ToString() + ".png"))
                { return true; }
                else
                { return false; }
            }
        ...
        */
        ...
        internal void DrawTroop(SpriteBatch spriteBatch, Troop troop, bool playerControlling)
        {
            if (troop.Scenario.ScenarioMap.TileWidth >= 50)
            {
                ...
                {
                    UIKind = "New1";
                    if (File.Exists(@"GameComponents\PersonPortrait\Images\Default\" + this.PersonID.ToString() + "t.jpg"))
                    { UIKind = "New2"; }
                    else if (File.Exists(@"GameComponents\PersonPortrait\Images\Player\" + this.PersonID.ToString() + "t.jpg"))
                    { UIKind = "New2"; }
                }
                if (UIKind == "Old")
                {                    
                ...
                if (UIKind == "New2")
                {
                    ...
                    {
                        spriteBatch.Draw(troop.Leader.TroopPortrait, this.ThePortraitDisplayPosition, null, Color.White, 0f, Vector2.Zero, SpriteEffects.None, 0.469f);
                        if (ShowFactionName2Background == "on")
                        {
